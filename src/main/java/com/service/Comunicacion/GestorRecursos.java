...
private int Lockeartodos(ArrayList<Integer> x,int id) {
        ...
        int i =0;
    for(Integer z:x){
        BalanzaBase bza = (BalanzaBase) BalanzaService.getInstance().Balanzas.getBalanza(z);
    ...
    private void iniciarRecursos(){
        ...
        BalanzaBase bzs = (BalanzaBase) BalanzaService.getInstance().Balanzas.getBalanza(1);
        int i=2;
        while(bzs.activity!=null){
            ...
            bzs =(BalanzaBase) BalanzaService.getInstance().Balanzas.getBalanza(i);
            i++;
        }
        // o:
    ...
    public void initpuertogestor(final ArrayList<Integer> arrayList, final int id){
        ...
        try {
            BalanzaBase x = (BalanzaBase) BalanzaService.getInstance().Balanzas.getBalanza(arrayList.get(0));
            band485 = false;
            if(x.activity!=null){
                band485 = x.band485;
                if(!band485){
                ...
                public void run() {
                    ...
                    while(isrunning[id]) {
                        for (Integer i = 0; arrayList.size() > i; i++) {
                            ...
                            BalanzaBase z = (BalanzaBase)BalanzaService.getInstance().Balanzas.getBalanza(arrayList.get(i));
